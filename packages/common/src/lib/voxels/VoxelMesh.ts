...
import type { CulledMesher } from './CulledMesher';
import type { Chunk, ChunkManager } from './ChunkManager';

function generateAmbientOcclusion(grid: number[]) {
	return [
...
function generateGrid(
	...
	const quad: Vector3[] = [];
	for (let r = 0; r < 4; r++) {
		const vertex: Vector3Tuple = vertices[indexes[r]];
		quad.push(new Vector3(vertex[0], vertex[1], vertex[2]));
	}
...
export class VoxelMesh {
	...
	meshed: ReturnType<(typeof CulledMesher)['prototype']['mesh']>;
	surfaceMesh: Mesh | undefined;
	...
	constructor(
		...
		const repeatUV: number[] = [];
		const subrects: number[] = [];
	...
	createSurfaceMesh(material: Material) {
